...
async def login(
    ...
    contrasena: str = Query(..., min_length=6, max_length=50, description="Contraseña"),
    db: Session = Depends(get_db)
):
    db_usuario = db.query(models.Usuarios).filter(models.Usuarios.email == email).first()
    if db_usuario and bcrypt.checkpw(contrasena.encode('utf-8'), db_usuario.contrasena.encode('utf-8')):
...
def cambiar_contrasena(
    ...
    if not usuario or not bcrypt.checkpw(contrasena_actual.encode('utf-8'), usuario.contrasena.encode('utf-8')):
